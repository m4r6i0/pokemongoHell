...
public class ActionBarDrawerToggle implements DrawerListener {
    ...
    public void onDrawerSlide(View drawerView, float slideOffset) {
        ...
        if (slideOffset > 0.5f) {
            glyphOffset = Math.max(glyphOffset, Math.max(0.0f, slideOffset - 0.5f) * Default.HTTP_REQUEST_BACKOFF_MULT);
        } else {
            glyphOffset = Math.min(glyphOffset, slideOffset * Default.HTTP_REQUEST_BACKOFF_MULT);
        }
