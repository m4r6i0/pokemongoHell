...
public class ManagedVariableManager {
    ...
    static class C09681 extends HashMap<Class<? extends ManagedVariable>, Class<? extends ManagedVariableModel>> {
        C09681() {
    ...
    class C09692 implements Action1<T> {
        final /* synthetic */ Listener val$listener;

        C09692(Listener listener) {
    ...
    class C09703 implements Action1<ManagedVariableModel> {
        final /* synthetic */ Class val$clazz;
        final /* synthetic */ Listener val$listener;
        final /* synthetic */ String val$tag;

        C09703(String str, Listener listener, Class cls) {
            this.val$tag = str;
            this.val$listener = listener;
            this.val$clazz = cls;
        }
    ...
    class C09714 implements Action1<Throwable> {
        final /* synthetic */ Listener val$listener;

        C09714(Listener listener) {
            this.val$listener = listener;
        }
    ...
    class C09725 implements Func1<ManagedVariableModel, Boolean> {
        final /* synthetic */ Class val$clazz;
        final /* synthetic */ String val$tag;

        C09725(Class cls, String str) {
            this.val$clazz = cls;
            this.val$tag = str;
        }
    ...
    public <T extends ManagedVariable> UpsightSubscription fetch(Class<T> clazz, String tag, Listener<T> listener) {
        ...
        synchronized (this.mCache) {
            ...
            if (cachedVariable != null) {
                subscription = Subscriptions.from(Observable.just(cachedVariable).observeOn(this.mCallbackScheduler).subscribe(new C09692(listener)));
            } else {
                subscription = Subscriptions.from(fetchDataStoreObservable(clazz, tag).subscribe(new C09703(tag, listener, clazz), new C09714(listener)));
            }
        }
        return subscription;
    }
    ...
    private <T extends ManagedVariable> Observable<? extends ManagedVariableModel> fetchDataStoreObservable(Class<T> clazz, String tag) {
        return this.mDataStore.fetchObservable((Class) sModelMap.get(clazz)).filter(new C09725(clazz, tag)).defaultIfEmpty(null);
    }
