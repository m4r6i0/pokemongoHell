        ...
        try {
            ...
            if (bundle != null && bundle.containsKey(KEY_SESSION)) {
                try {
                    ...
                    this.mLogger.m207e(Upsight.LOG_TAG, String.format("Unexpected error: Class: %s was not found.", new Object[]{session}), e);
                } catch (IllegalAccessException e2) {
                    this.mLogger.m207e(Upsight.LOG_TAG, String.format("Unexpected error: Class: %s does not have public access.", new Object[]{session}), e2);
                } catch (InstantiationException e3) {
                    this.mLogger.m207e(Upsight.LOG_TAG, String.format("Unexpected error: Class: %s could not be instantiated", new Object[]{session}), e3);
                }
            }
        } catch (NameNotFoundException e4) {
            this.mLogger.m207e(Upsight.LOG_TAG, "Unexpected error: Package name missing!?", e4);
        }
    }
    ...
    private boolean applyConfiguration(String jsonConfig) {
        try {
            ...
            if (config == null || !config.isValid()) {
                this.mLogger.m213w(LOG_TAG, "New config is invalid", new Object[0]);
                return false;
            } else if (config.equals(this.mCurrentConfig)) {
                this.mLogger.m213w(LOG_TAG, "New config ignored because it is equal to current config", new Object[0]);
                return true;
            } else {
            ...
            this.mLogger.m207e(LOG_TAG, "Failed to apply new config", e);
            return false;
        }
    }
