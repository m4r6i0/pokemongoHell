...
public class VASTPlayer {
    ...
    public void loadVastResponseViaURL(String urlString) {
        ...
        if (NetworkTools.connectedToInternet(this.context)) {
            new Thread(new C10651(urlString)).start();
        } else {
    ...
    public void loadVastResponseViaXML(String xmlData) {
        this.vastModel = null;
        if (NetworkTools.connectedToInternet(this.context)) {
            new Thread(new C10662(xmlData)).start();
        } else {
            sendError(ERROR_NO_NETWORK);
        }
    }
        ...
        if (this.vastModel != null) {
            ...
            if (components.length == 3) {
                try {
                    ...
                    FuseLog.m239v(TAG, "Overriding server sent skip offset with VAST offset from XML: " + timeoutMs);
                    this.skipOffset = timeoutMs;
                } catch (NumberFormatException e) {
                    FuseLog.m238i(TAG, "Could not parse skip offset from xml: " + xmlSkipOffset + ", using cb_ms instead");
                }
            }
    ...
    private void sendError(int error) {
        if (this.listener != null) {
            ((Activity) this.context).runOnUiThread(new C10673(error));
        }
    }
}
