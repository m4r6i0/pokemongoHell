...
public class VASTActivity extends Activity implements OnCompletionListener, OnErrorListener, OnPreparedListener, OnSeekCompleteListener, OnVideoSizeChangedListener, Callback, Listener {
    ...
    class C10701 implements OnTouchListener {
        C10701() {
        }
    ...
    class C10712 implements OnTouchListener {
        C10712() {
        }
    ...
    class C10723 implements OnTouchListener {
        C10723() {
        }
    ...
    class C10744 extends TimerTask {
    ...
    class C10776 extends TimerTask {
        int maxAmountInList;
        ...
        public void run() {
            if (VASTActivity.this.mMediaPlayer != null) {
                ...
                try {
                    int curPos = VASTActivity.this.mMediaPlayer.getCurrentPosition();
                    VASTActivity.this.runOnUiThread(new C10761());
    ...
    private void createOverlay(LayoutParams params) {
        ...
        this.mOverlay.setOnTouchListener(new C10701());
    ...
    private void createPlayerControls() {
        ...
        if (button != null) {
            ...
            button.setOnTouchListener(new C10712());
        }
        this.mPlayerControls.setSkipButtonListener(new C10723());
    ...
    private void processClickThroughEvent() {
        String clickThroughUrl = this.mVastModel.getVideoClicks().getClickThrough();
        FuseLog.m239v(TAG, "clickThrough url: " + clickThroughUrl);
    ...
    public void onOpenMRaidUrl(String url) {
        FuseLog.m238i(TAG, "Opening MRAID Postroll click through link: " + url);
    ...
    private void calculateAspectRatio() {
        ...
        FuseLog.m239v(TAG, " video size:  " + this.mVideoWidth + "x" + this.mVideoHeight);
        FuseLog.m239v(TAG, "surface size: " + surfaceWidth + "x" + surfaceHeight);
    }
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        FuseLog.m240w(TAG, "Shutting down Activity due to Media Player errors: WHAT:" + what + ": EXTRA:" + extra + UpsightEndpoint.SIGNED_MESSAGE_SEPARATOR);
        VASTPlayer.currentPlayer.listener.vastError(VASTPlayer.ERROR_VIDEO_PLAYBACK);
        processErrorEvent();
        closeClicked();
        return true;
    ...
    private void startToolBarTimer() {
        if (this.mQuartile != 4) {
            if (this.mMediaPlayer != null && this.mMediaPlayer.isPlaying()) {
                ...
                this.mToolBarTimer.schedule(new C10744(), TOOLBAR_HIDE_DELAY);
                this.mPlayerControls.setVisibility(0);
            }
            if (this.mIsVideoPaused) {
                activateButtons(true);
    ...
    private void startTrackingEventTimer() {
        ...
        if (!this.mIsCompleted) {
            ...
            this.mTrackingEventTimer.scheduleAtFixedRate(new C10755(videoDuration, events), 0, QUARTILE_TIMER_INTERVAL);
        }
    }
    ...
    private void startVideoProgressTimer() {
        ...
        this.mStartVideoProgressTimer.schedule(new C10776(), 0, VIDEO_PROGRESS_TIMER_INTERVAL);
    }
