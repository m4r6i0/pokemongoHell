...
public class MRAIDView extends RelativeLayout {
    ...
    class C10521 extends SimpleOnGestureListener {
        C10521() {
        }
    ...
    class C10532 extends WebView {
        ...
        public void onConfigurationChanged(Configuration newConfig) {
            ...
            MRAIDLog.m249v(TAG, "onConfigurationChanged " + (newConfig.orientation == MRAIDView.STATE_DEFAULT ? "portrait" : "landscape"));
        ...
        protected void onVisibilityChanged(View changedView, int visibility) {
            ...
            MRAIDLog.m249v(TAG, "onVisibilityChanged " + MRAIDView.getVisibilityString(visibility));
            if (MRAIDView.this.isInterstitial) {
        ...
        protected void onWindowVisibilityChanged(int visibility) {
            ...
            MRAIDLog.m249v(TAG, "onWindowVisibilityChanged " + MRAIDView.getVisibilityString(visibility) + " (actual " + MRAIDView.getVisibilityString(actualVisibility) + ")");
            if (MRAIDView.this.isInterstitial) {
    ...
    class C10543 implements OnTouchListener {
        C10543() {
        }
    ...
    class C10586 implements Runnable {
        ...
        public void run() {
            ...
            if (TextUtils.isEmpty(content)) {
                ...
                ((Activity) MRAIDView.this.context).runOnUiThread(new C10571(content));
            }
        }
    }
    ...
    class C10608 implements Runnable {
        final /* synthetic */ boolean val$sendCallback;
    ...
    class C10619 implements Runnable {
        final /* synthetic */ boolean val$sendCallback;
    ...
    private class MRAIDWebChromeClient extends WebChromeClient {
        ...
        public boolean onConsoleMessage(ConsoleMessage cm) {
            if (!cm.message().contains("Uncaught ReferenceError")) {
                MRAIDLog.m247i("JS console", cm.message() + (cm.sourceId() == null ? BuildConfig.FLAVOR : " at " + cm.sourceId()) + UpsightEndpoint.SIGNED_MESSAGE_SEPARATOR + cm.lineNumber());
            }
            return true;
        }

        public boolean onJsAlert(WebView view, String url, String message, JsResult result) {
            MRAIDLog.m249v("JS alert", message);
            return handlePopups(result);
        }

        public boolean onJsConfirm(WebView view, String url, String message, JsResult result) {
            MRAIDLog.m249v("JS confirm", message);
            return handlePopups(result);
        }

        public boolean onJsPrompt(WebView view, String url, String message, String defaultValue, JsPromptResult result) {
            MRAIDLog.m249v("JS prompt", message);
            return handlePopups(result);
        }
    ...
    private class MRAIDWebViewClient extends WebViewClient {
        ...
        public void onPageFinished(WebView view, String url) {
            MRAIDLog.m249v(MRAIDView.TAG, "onPageFinished: " + url);
            ...
            if (MRAIDView.this.isExpandingPart2) {
                ...
                MRAIDView.this.handler.post(new C10621());
            }
        }

        public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
            MRAIDLog.m249v(MRAIDView.TAG, "onReceivedError: " + description);
            super.onReceivedError(view, errorCode, description, failingUrl);
        }

        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            MRAIDLog.m249v(MRAIDView.TAG, "shouldOverrideUrlLoading: " + url);
    ...
    protected MRAIDView(Context context, String baseUrl, String data, int backgroundColor, String[] supportedNativeFeatures, MRAIDViewListener listener, MRAIDNativeFeatureListener nativeFeatureListener, boolean isInterstitial) {
        ...
        this.gestureDetector = new GestureDetector(getContext(), new C10521());
    ...
    private WebView createWebView() {
        WebView wv = new C10532(this.context);
        ...
        wv.setOnTouchListener(new C10543());
    ...
    private void parseCommandUrl(String commandUrl) {
        MRAIDLog.m249v(TAG, "parseCommandUrl " + commandUrl);
    ...
    private void close(boolean sendCallback) {
        ...
        this.handler.post(new C10554(sendCallback));
    }
    ...
    private void rewardComplete() {
        ...
        this.handler.post(new C10565());
    }

    private void createCalendarEvent(String eventJSON) {
        MRAIDLog.m249v("MRAIDView-JS callback", "createCalendarEvent " + eventJSON);
    ...
    private void expand(String url) {
        MRAIDLog.m249v("MRAIDView-JS callback", "expand " + (url != null ? url : "(1-part)"));
        ...
        try {
            ...
            new Thread(new C10586(url), "2-part-content").start();
    ...
    private void open(String url) {
        try {
            url = URLDecoder.decode(url, "UTF-8");
            MRAIDLog.m249v("MRAIDView-JS callback", "open " + url);
            if (this.nativeFeatureListener == null) {
                return;
            }
    ...
    private void playVideo(String url) {
        try {
            url = URLDecoder.decode(url, "UTF-8");
            MRAIDLog.m249v("MRAIDView-JS callback", "playVideo " + url);
            if (this.nativeFeatureListener != null) {
    ...
    private void setOrientationProperties(Map<String, String> properties) {
        ...
        MRAIDLog.m249v("MRAIDView-JS callback", "setOrientationProperties " + allowOrientationChange + " " + forceOrientation);
    ...
    private void setResizeProperties(Map<String, String> properties) {
        ...
        MRAIDLog.m249v("MRAIDView-JS callback", "setResizeProperties " + width + " " + height + " " + offsetX + " " + offsetY + " " + customClosePosition + " " + allowOffscreen);
    ...
    private void storePicture(String url) {
        try {
            url = URLDecoder.decode(url, "UTF-8");
            MRAIDLog.m249v("MRAIDView-JS callback", "storePicture " + url);
            if (this.nativeFeatureListener != null) {
    ...
    private void useCustomClose(String useCustomCloseString) {
        MRAIDLog.m249v("MRAIDView-JS callback", "useCustomClose " + useCustomCloseString);
    ...
    private String getStringFromUrl(String url) {
        ...
        try {
            ...
            MRAIDLog.m249v(TAG, "response code " + responseCode);
            if (responseCode == 200) {
                MRAIDLog.m249v(TAG, "getContentLength " + conn.getContentLength());
                ...
                content = sb.toString();
                MRAIDLog.m249v(TAG, "getStringFromUrl ok, length=" + content.length());
            }
            conn.disconnect();
            if (is == null) {
                return content;
            ...
            MRAIDLog.m247i(TAG, "getStringFromUrl failed " + e2.getLocalizedMessage());
            if (is == null) {
                return null;
            }
            try {
    ...
    private void closeFromExpanded(boolean sendCallback) {
        ...
        this.handler.post(new C10608(sendCallback));
    }
    ...
    private void closeFromResized(boolean sendCallback) {
        ...
        removeResizeView();
        addView(this.webView);
        this.handler.post(new C10619(sendCallback));
    }
    ...
    private void setCurrentPosition() {
        ...
        MRAIDLog.m249v(TAG, "setCurrentPosition [" + x + "," + y + "] (" + width + "x" + height + ")");
    ...
    private void setDefaultPosition() {
        ...
        MRAIDLog.m249v(TAG, "setDefaultPosition [" + x + "," + y + "] (" + width + "x" + height + ")");
    ...
    private void setMaxSize() {
        ...
        MRAIDLog.m249v(TAG, "setMaxSize " + width + "x" + height);
    ...
    private void setScreenSize() {
        ...
        MRAIDLog.m249v(TAG, "setScreenSize " + width + "x" + height);
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        super.onConfigurationChanged(newConfig);
        MRAIDLog.m249v(TAG, "onConfigurationChanged " + (newConfig.orientation == STATE_DEFAULT ? "portrait" : "landscape"));
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        super.onVisibilityChanged(changedView, visibility);
        MRAIDLog.m249v(TAG, "onVisibilityChanged " + getVisibilityString(visibility));
    ...
    protected void onWindowVisibilityChanged(int visibility) {
        super.onWindowVisibilityChanged(visibility);
        int actualVisibility = getVisibility();
        MRAIDLog.m249v(TAG, "onWindowVisibilityChanged " + getVisibilityString(visibility) + " (actual " + getVisibilityString(actualVisibility) + ")");
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        super.onLayout(changed, left, top, right, bottom);
        MRAIDLog.m249v(TAG, "onLayout (" + this.state + ") " + changed + " " + left + " " + top + " " + right + " " + bottom);
    ...
    private void onLayoutWebView(WebView wv, boolean changed, int left, int top, int right, int bottom) {
        ...
        MRAIDLog.m249v(TAG, "onLayoutWebView " + (wv == this.webView ? "1 " : "2 ") + isCurrent + " (" + this.state + ") " + changed + " " + left + " " + top + " " + right + " " + bottom);
        if (!isCurrent) {
    ...
    private void calculateScreenSize() {
        ...
        MRAIDLog.m249v(TAG, "calculateScreenSize orientation " + (isPortrait ? "portrait" : "landscape"));
        int width = this.displayMetrics.widthPixels;
        int height = this.displayMetrics.heightPixels;
        MRAIDLog.m249v(TAG, "calculateScreenSize screen size " + width + "x" + height);
    ...
    private void calculateMaxSize() {
        ...
        MRAIDLog.m249v(TAG, "calculateMaxSize frame [" + frame.left + "," + frame.top + "][" + frame.right + "," + frame.bottom + "] (" + frame.width() + "x" + frame.height() + ")");
        int statusHeight = frame.top;
        this.contentViewTop = window.findViewById(16908290).getTop();
        int titleHeight = this.contentViewTop - statusHeight;
        MRAIDLog.m249v(TAG, "calculateMaxSize statusHeight " + statusHeight);
        MRAIDLog.m249v(TAG, "calculateMaxSize titleHeight " + titleHeight);
        ...
        MRAIDLog.m249v(TAG, "calculateMaxSize max size " + width + "x" + height);
    ...
    private void calculatePosition(boolean isCurrentWebView) {
        ...
        MRAIDLog.m249v(TAG, "calculatePosition " + name + " locationOnScreen [" + x + "," + y + "]");
        MRAIDLog.m249v(TAG, "calculatePosition " + name + " contentViewTop " + this.contentViewTop);
        if (y < this.contentViewTop) {
            MRAIDLog.m249v(TAG, "calculatePosition " + name + " y < contentViewTop, returning");
            return;
        }
        y -= this.contentViewTop;
        int width = view.getWidth();
        int height = view.getHeight();
        MRAIDLog.m249v(TAG, "calculatePosition " + name + " position [" + x + "," + y + "] (" + width + "x" + height + ")");
