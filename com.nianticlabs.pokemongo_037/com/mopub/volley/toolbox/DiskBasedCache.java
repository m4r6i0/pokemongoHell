...
public class DiskBasedCache implements Cache {
    ...
    public synchronized void clear() {
        ...
        this.mEntries.clear();
        this.mTotalSize = 0;
        VolleyLog.m42d("Cache cleared.", new Object[0]);
    }
            ...
            if (entry2 != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        cis = cis2;
                        try {
                            VolleyLog.m42d("%s: %s", file.getAbsolutePath(), e.toString());
                            remove(key);
                            if (cis != null) {
                                try {
                                    cis.close();
    ...
    L_0x0011:
        ...
        com.mopub.volley.VolleyLog.m43e(r9, r10);	 Catch:{ all -> 0x006e }
    L_0x0022:
        monitor-exit(r13);
        return;
    L_0x0024:
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (e.writeHeader(fos)) {
                ...
                fos.close();
                VolleyLog.m42d("Failed to write header for %s", file.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e2) {
            if (!file.delete()) {
                VolleyLog.m42d("Could not clean up file %s", file.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            VolleyLog.m42d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        if (this.mTotalSize + ((long) neededSpace) >= ((long) this.mMaxCacheSizeInBytes)) {
            if (VolleyLog.DEBUG) {
                VolleyLog.m45v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (iterator.hasNext()) {
                ...
                if (getFileForKey(e.key).delete()) {
                    ...
                    VolleyLog.m42d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
                }
                iterator.remove();
                prunedFiles++;
            ...
            if (VolleyLog.DEBUG) {
                VolleyLog.m45v("pruned %d files, %d bytes, %d ms", Integer.valueOf(prunedFiles), Long.valueOf(this.mTotalSize - before), Long.valueOf(SystemClock.elapsedRealtime() - startTime));
            }
        }
    }
