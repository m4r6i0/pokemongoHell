...
public class PluginException extends Throwable {
    ...
    public static PluginException createUnityException(String message, String stack) {
        ...
        try {
            ...
            dx.m776a("Unable to log unity exception <" + message + "> " + stack, th);
        }
    ...
    private static PluginException unsafeCreateUnityException(String message, String stack) {
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            if (trim2.length() != 0) {
                ...
                if (lastIndexOf == -1) {
                    dx.m779b("Unable to parse unity exception.  No class and method found for frame frame <" + trim2 + ">" + stack);
                    return null;
                } else if (lastIndexOf == str.length() - 1) {
                    dx.m779b("Unable to parse unity exception.  Method is zero length for frame <" + trim2 + ">" + stack);
                    return null;
                } else {
