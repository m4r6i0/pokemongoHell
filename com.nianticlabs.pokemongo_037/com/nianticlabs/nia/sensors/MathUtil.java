...
public final class MathUtil {
    ...
    public static float ease(float t) {
        return (float) Math.sin((((double) Math.max(Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, t), 0.0f)) * 3.141592653589793d) / 2.0d);
    }

    public static float saturate(float f) {
        return clamp(f, 0.0f, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    }
    ...
    public static float[] quadraticBezier(float x0, float y0, float x1, float y1, float x2, float y2, int N) {
        ...
        for (int i = 0; i < N; i++) {
            float t = ((float) i) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT / ((float) (N - 1)));
            a[i * 2] = ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - t) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - t)) * x0) + ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - t) * Default.HTTP_REQUEST_BACKOFF_MULT) * t) * x1)) + ((t * t) * x2);
            a[(i * 2) + 1] = ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - t) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - t)) * y0) + ((((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - t) * Default.HTTP_REQUEST_BACKOFF_MULT) * t) * y1)) + ((t * t) * y2);
        }
        return a;
    }
