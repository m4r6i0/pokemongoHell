...
public class VolleyLog {
    ...
    static class MarkerLog {
        ...
        public synchronized void finish(String header) {
            ...
            if (getTotalDuration() > 0) {
                ...
                VolleyLog.m16d("(%-4d ms) %s", Long.valueOf(duration), header);
                for (Marker marker : this.mMarkers) {
                    VolleyLog.m16d("(+%-4d) [%2d] %s", Long.valueOf(marker.time - prevTime), Long.valueOf(marker.thread), marker.name);
    ...
    public static void setTag(String tag) {
        m16d("Changing log tag to %s", tag);
        TAG = tag;
        DEBUG = Log.isLoggable(TAG, 2);
    }

    public static void m19v(String format, Object... args) {
        if (DEBUG) {
            Log.v(TAG, buildMessage(format, args));
        }
    }

    public static void m16d(String format, Object... args) {
        Log.d(TAG, buildMessage(format, args));
    }

    public static void m17e(String format, Object... args) {
        Log.e(TAG, buildMessage(format, args));
    }

    public static void m18e(Throwable tr, String format, Object... args) {
