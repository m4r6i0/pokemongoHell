...
public final class OperatorSequenceEqual {
    ...
    static class C14201 implements Func1<T, Object> {
        C14201() {
        }
    ...
    static class C14212 implements Func2<Object, Object, Boolean> {
    ...
    static <T> Observable<Object> materializeLite(Observable<T> source) {
        return Observable.concat(source.map(new C14201()), Observable.just(LOCAL_ONCOMPLETED));
    }
    ...
    public static <T> Observable<Boolean> sequenceEqual(Observable<? extends T> first, Observable<? extends T> second, Func2<? super T, ? super T, Boolean> equality) {
        return Observable.zip(materializeLite(first), materializeLite(second), new C14212(equality)).all(UtilityFunctions.identity());
    }
}
