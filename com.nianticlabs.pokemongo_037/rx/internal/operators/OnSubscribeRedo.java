...
public final class OnSubscribeRedo<T> implements OnSubscribe<T> {
    ...
    static class C13321 implements Func1<Observable<? extends Notification<?>>, Observable<?>> {

        /* renamed from: rx.internal.operators.OnSubscribeRedo.1.1 */
        class C13311 implements Func1<Notification<?>, Notification<?>> {
            C13311() {
            }
        ...
        public Observable<?> call(Observable<? extends Notification<?>> ts) {
            return ts.map(new C13311());
        }
    }

    /* renamed from: rx.internal.operators.OnSubscribeRedo.2 */
    class C13342 implements Action0 {
        ...
        class C13331 extends Subscriber<T> {
            boolean done;
            ...
            public void onCompleted() {
                if (!this.done) {
                    this.done = true;
                    unsubscribe();
                    C13342.this.val$terminals.onNext(Notification.createOnCompleted());
                }
            }
            ...
            public void onError(Throwable e) {
                if (!this.done) {
                    this.done = true;
                    unsubscribe();
                    C13342.this.val$terminals.onNext(Notification.createOnError(e));
                }
            }

            public void onNext(T v) {
                if (!this.done) {
                    C13342.this.val$child.onNext(v);
                    decrementConsumerCapacity();
                    C13342.this.val$arbiter.produced(1);
                }
            }
            ...
            private void decrementConsumerCapacity() {
                long cc;
                do {
                    cc = C13342.this.val$consumerCapacity.get();
                    if (cc == Long.MAX_VALUE) {
                        return;
                    }
                } while (!C13342.this.val$consumerCapacity.compareAndSet(cc, cc - 1));
            }

            public void setProducer(Producer producer) {
                C13342.this.val$arbiter.setProducer(producer);
            }
        }

        C13342(Subscriber subscriber, BehaviorSubject behaviorSubject, ProducerArbiter producerArbiter, AtomicLong atomicLong, SerialSubscription serialSubscription) {
        ...
        public void call() {
            if (!this.val$child.isUnsubscribed()) {
                Subscriber<T> terminalDelegatingSubscriber = new C13331();
    ...
    class C13363 implements Operator<Notification<?>, Notification<?>> {

        /* renamed from: rx.internal.operators.OnSubscribeRedo.3.1 */
        class C13351 extends Subscriber<Notification<?>> {
            final /* synthetic */ Subscriber val$filteredTerminals;

            C13351(Subscriber x0, Subscriber subscriber) {
        ...
        public Subscriber<? super Notification<?>> call(Subscriber<? super Notification<?>> filteredTerminals) {
            return new C13351(filteredTerminals, filteredTerminals);
        }
    }

    /* renamed from: rx.internal.operators.OnSubscribeRedo.4 */
    class C13384 implements Action0 {
        final /* synthetic */ Subscriber val$child;
        final /* synthetic */ AtomicLong val$consumerCapacity;
        ...
        class C13371 extends Subscriber<Object> {
            C13371(Subscriber x0) {
                super(x0);
            }

            public void onCompleted() {
                C13384.this.val$child.onCompleted();
            }

            public void onError(Throwable e) {
                C13384.this.val$child.onError(e);
            }

            public void onNext(Object t) {
                if (!C13384.this.val$child.isUnsubscribed()) {
                    if (C13384.this.val$consumerCapacity.get() > 0) {
                        C13384.this.val$worker.schedule(C13384.this.val$subscribeToSource);
                    } else {
                        C13384.this.val$resumeBoundary.compareAndSet(false, true);
                    }
                }
            }
        ...
        C13384(Observable observable, Subscriber subscriber, AtomicLong atomicLong, Worker worker, Action0 action0, AtomicBoolean atomicBoolean) {
        ...
        public void call() {
            this.val$restarts.unsafeSubscribe(new C13371(this.val$child));
        }
    }

    /* renamed from: rx.internal.operators.OnSubscribeRedo.5 */
    class C13395 implements Producer {
        final /* synthetic */ ProducerArbiter val$arbiter;
        final /* synthetic */ AtomicLong val$consumerCapacity;
        final /* synthetic */ AtomicBoolean val$resumeBoundary;
        final /* synthetic */ Action0 val$subscribeToSource;
        ...
        C13395(AtomicLong atomicLong, ProducerArbiter producerArbiter, AtomicBoolean atomicBoolean, Worker worker, Action0 action0) {
            this.val$consumerCapacity = atomicLong;
            this.val$arbiter = producerArbiter;
            this.val$resumeBoundary = atomicBoolean;
            this.val$worker = worker;
    ...
    public static final class RedoFinite implements Func1<Observable<? extends Notification<?>>, Observable<?>> {
        ...
        class C13401 implements Func1<Notification<?>, Notification<?>> {
            int num;
        ...
        public Observable<?> call(Observable<? extends Notification<?>> ts) {
            return ts.map(new C13401()).dematerialize();
        }
    }
    ...
    public static final class RetryWithPredicate implements Func1<Observable<? extends Notification<?>>, Observable<? extends Notification<?>>> {
        ...
        class C13411 implements Func2<Notification<Integer>, Notification<?>, Notification<Integer>> {
            C13411() {
            }
        ...
        public Observable<? extends Notification<?>> call(Observable<? extends Notification<?>> ts) {
            return ts.scan(Notification.createOnNext(Integer.valueOf(0)), new C13411());
        }
    }

    static {
        REDO_INFINITE = new C13321();
    }
    ...
    public void call(Subscriber<? super T> child) {
        ...
        Action0 subscribeToSource = new C13342(child, terminals, arbiter, consumerCapacity, sourceSubscriptions);
        worker.schedule(new C13384((Observable) this.controlHandlerFunction.call(terminals.lift(new C13363())), child, consumerCapacity, worker, subscribeToSource, atomicBoolean));
        child.setProducer(new C13395(consumerCapacity, arbiter, atomicBoolean, worker, subscribeToSource));
    }
}
