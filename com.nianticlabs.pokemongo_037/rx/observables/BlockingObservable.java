...
public final class BlockingObservable<T> {
    ...
    private final Observable<? extends T> f921o;

    /* renamed from: rx.observables.BlockingObservable.1 */
    class C15121 extends Subscriber<T> {
        final /* synthetic */ AtomicReference val$exceptionFromOnError;
        final /* synthetic */ CountDownLatch val$latch;
        final /* synthetic */ Action1 val$onNext;

        C15121(CountDownLatch countDownLatch, AtomicReference atomicReference, Action1 action1) {
    ...
    class C15132 implements Iterable<T> {
        C15132() {
        }
    ...
    class C15143 extends Subscriber<T> {
        final /* synthetic */ CountDownLatch val$latch;
        final /* synthetic */ AtomicReference val$returnException;
        final /* synthetic */ AtomicReference val$returnItem;

        C15143(CountDownLatch countDownLatch, AtomicReference atomicReference, AtomicReference atomicReference2) {
            this.val$latch = countDownLatch;
    ...
    class C15154 extends Subscriber<T> {
        final /* synthetic */ CountDownLatch val$cdl;
        final /* synthetic */ Throwable[] val$error;

        C15154(Throwable[] thArr, CountDownLatch countDownLatch) {
    ...
    class C15165 extends Subscriber<T> {
        final /* synthetic */ NotificationLite val$nl;
        final /* synthetic */ BlockingQueue val$queue;

        C15165(BlockingQueue blockingQueue, NotificationLite notificationLite) {
    ...
    class C15176 extends Subscriber<T> {
        final /* synthetic */ NotificationLite val$nl;
        final /* synthetic */ BlockingQueue val$queue;
        final /* synthetic */ Producer[] val$theProducer;

        C15176(BlockingQueue blockingQueue, NotificationLite notificationLite, Producer[] producerArr) {
            this.val$queue = blockingQueue;
            this.val$nl = notificationLite;
    ...
    class C15187 implements Action0 {
        final /* synthetic */ BlockingQueue val$queue;

        C15187(BlockingQueue blockingQueue) {
            this.val$queue = blockingQueue;
        }
    ...
    class C15198 implements Action1<Throwable> {
        C15198() {
        }
    ...
    class C15209 implements Observer<T> {
        final /* synthetic */ Action0 val$onCompleted;
        final /* synthetic */ Action1 val$onError;
        final /* synthetic */ Action1 val$onNext;

        C15209(Action1 action1, Action1 action12, Action0 action0) {
            this.val$onNext = action1;
    ...
    public void forEach(Action1<? super T> onNext) {
        ...
        BlockingUtils.awaitForComplete(latch, this.f921o.subscribe(new C15121(latch, exceptionFromOnError, onNext)));
    ...
    public Iterator<T> getIterator() {
        return BlockingOperatorToIterator.toIterator(this.f921o);
    }

    public T first() {
        return blockForSingle(this.f921o.first());
    }

    public T first(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f921o.first(predicate));
    }

    public T firstOrDefault(T defaultValue) {
        return blockForSingle(this.f921o.map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T firstOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f921o.filter(predicate).map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T last() {
        return blockForSingle(this.f921o.last());
    }

    public T last(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f921o.last(predicate));
    }

    public T lastOrDefault(T defaultValue) {
        return blockForSingle(this.f921o.map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public T lastOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f921o.filter(predicate).map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public Iterable<T> mostRecent(T initialValue) {
        return BlockingOperatorMostRecent.mostRecent(this.f921o, initialValue);
    }

    public Iterable<T> next() {
        return BlockingOperatorNext.next(this.f921o);
    }

    public Iterable<T> latest() {
        return BlockingOperatorLatest.latest(this.f921o);
    }

    public T single() {
        return blockForSingle(this.f921o.single());
    }

    public T single(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f921o.single(predicate));
    }

    public T singleOrDefault(T defaultValue) {
        return blockForSingle(this.f921o.map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public T singleOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f921o.filter(predicate).map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public Future<T> toFuture() {
        return BlockingOperatorToFuture.toFuture(this.f921o);
    }
    ...
    private T blockForSingle(Observable<? extends T> observable) {
        ...
        CountDownLatch latch = new CountDownLatch(1);
        BlockingUtils.awaitForComplete(latch, observable.subscribe(new C15143(latch, returnException, returnItem)));
    ...
    public void subscribe() {
        ...
        BlockingUtils.awaitForComplete(cdl, this.f921o.subscribe(new C15154(error, cdl)));
        Throwable e = error[0];
        if (e == null) {
            return;
        }
    ...
    public void subscribe(Observer<? super T> observer) {
        ...
        Subscription s = this.f921o.subscribe(new C15165(queue, nl));
        while (true) {
            try {
                Object o = queue.poll();
                if (o == null) {
    ...
    public void subscribe(Action1<? super T> onNext) {
        subscribe(onNext, new C15198(), Actions.empty());
    }
    ...
    public void subscribe(Action1<? super T> onNext, Action1<? super Throwable> onError, Action0 onCompleted) {
        subscribe(new C15209(onNext, onError, onCompleted));
    }
}
